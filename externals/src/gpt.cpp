...
std::vector<std::string> GPT::str::split2(std::string in, char splitter)
{
...
void GPT::Math::Vector::set_position(float first, ...)
{
	...
	for (size_t i = 1; i < size; i++) {
		float arg = (float)va_arg(args, float);
		m_pos[i] = arg;
	}
	va_end(args);
}
...
GPT::Math::Vector2 GPT::Math::rotatePoint(Vector2 p, Vector2 c, float angle)
{

	Vector2 output;
	output.x = (p.x - c.x) * std::cosf(angle) - (p.y - c.y) * sinf(angle) + c.x;
	output.y = (p.y - c.y) * std::cosf(angle) + (p.x - c.x) * sinf(angle) + c.y;
	return output;
}
...
GPT::Math::Vector2 GPT::Math::Vector::toVec2()
{
	if (get_dimension() != 2)
		return { -1, 0 };
	GPT::Math::Vector2 output = {m_pos[0], m_pos[1]};
	return output;
}
...
void GPT::phy::Physics::Initialisation(uint8_t dimensions, bool shouldBeAffectedByGravity, float gravity)
{
	isAffectedByGravity = shouldBeAffectedByGravity;
	gravityAcceleration = gravity;
	pos.set_dimension(dimensions);
	size.set_dimension(dimensions);
...
void GPT::log::Print(std::ofstream* file, std::string buffer, bool checkNewLine)
{
	...
	if (checkNewLine && buffer[buffer.size()-1] != '\n')
	{
		*file << GetFormat() << buffer << std::endl;
		std::cout << GetFormat() << buffer << std::endl;
	}
	else
	{
